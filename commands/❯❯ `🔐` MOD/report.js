...
module.exports = {
    ...
    run: (client, message, arguments) => {
        ...
        if (!target) {
            message.channel.send(`👎 **| Mention a user to report.**`)
        } else if (!reason) {
            message.channel.send(`👎 **| Mention the reason of your report.**`)
        } else {
            message.channel.send(`👍 **| ${target.user.tag} has been reported.**`)
